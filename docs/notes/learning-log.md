...
**ヘッダの自己完結性を検査する仕組みを常設した。** 前回の「次にやること」3番。
ブランチ `chore/header-self-containment-check`、理由ごとに6コミット。

- **サイクルを解いた** — `vulkan_context.h` の `#include "core/command_buffer.h"` を削除
  （`std::shared_ptr<CommandBuffer>` しか使っておらず、前方宣言で足りる）。
  代わりに4本の `.cpp` が自分で include。**外すと本当に壊れることを実測**した
  （9 / 11 / 5 / 2 件のエラー。`incomplete type` への member access と `make_shared` の `sizeof`）
- **ヘッダ8本に不足 include 10行**（`<cstdint>` 6・`<memory>` 2・`<vector>` 1・`<cstddef>` 1）。
  **8/15 に scratchpad のコピーで測った数字が、実ツリーでそのまま再現した**
- **`stage1_header_check` ターゲットを追加**（`lib/stage1/CMakeLists.txt` を新設）。
  ヘッダを `LANGUAGE CXX` で単独 TU としてコンパイルし、`compile_commands.json` に14本を載せる。
  一覧は `file(GLOB_RECURSE ... CONFIGURE_DEPENDS)` で自動収集（登録忘れが未検査に化けないため）。
  通常のビルドに含めたので、レベル1「単独で通るか」は常時保証される
- **実測**: 全38TU（`.cpp` 24 + ヘッダ14）で `run-clang-tidy` **0件**、クリーンビルドで
  **警告ゼロ**、`clang-format --dry-run --Werror` 全31ファイル差分なし。各アプリの実行も確認済み

**予定になかった指摘が2種類出た（どちらも対処済み）。**

- **`IWYU pragma: keep` を3件**。サイクルを解いて `.cpp` に include を移した結果、
  今度は `misc-include-cleaner` が「未使用」と言い出した。`auto& commandBuffer = ...` で
  受けていて `CommandBuffer` の綴りが現れないため。検査を切らずに、その行だけを名指しで止めた
- **`clang-analyzer-optin.cplusplus.VirtualCall` 4件**。`~DepthBuffer() { Cleanup(); }` の形。
  `DepthBuffer::Cleanup()` と修飾して解消した（挙動もコード生成も不変）。
  8/15 の CRTP は「直すと設計が変わる」から検査を切ったが、今回は設計も挙動も動かないので
  **切らずに直す**判断

### 現在地: 4章完了・振り返り中。5章は未着手（前回から変わらず）

`run-clang-tidy` は0件。**今回からヘッダ14本は「include されている側」ではなく
「main file」としても解析される**。7/30 に決めた「推移的インクルードは行わない」が、
初めて `.cpp` とヘッダの両方で機械的に守られている状態。
...
3. Windows 環境でもビルドと clang-tidy を通す（今回 `stage1_header_check` が増えたので、
   `LANGUAGE CXX` が MSVC で `/TP` になること、`-Wno-pragma-once-outside-header` が
   `$<CXX_COMPILER_ID:GNU,Clang,AppleClang>` で正しく外れることの確認も兼ねる）

### 気づき

- **道具を1つ増やすと、見える範囲だけでなく「鳴り方」も増える。** 今回の新規指摘は2種類とも、
  ヘッダを main file にしたから見えたもの。`clang-analyzer` は main file のトップレベル関数を
  起点に解析するので、ヘッダ内に定義されたデストラクタは、include されている側では
  一度も解析されていなかった。**0件は「全部見た結果0件」ではなく「見た範囲で0件」**
  （8/12「main file しか見ない」→ 8/14「参照束縛は DeadStores の対象外」→
  8/15「`auto` は綴られない」に続く4例目）
- **同じ `auto` の性質が、今度は逆向きに出た。** 8/15 は「`auto` のせいで include が
  要求されない」（検出漏れ）だったが、今回は「`auto` のせいで必要な include が
  未使用と言われる」（誤検出）。**道具の限界は1つでも、症状は文脈で反転する**
- **デストラクタ内の仮想呼び出しは、`virtual` と書いてあっても仮想ではない。**
  破棄中は動的型が「いま走っているデストラクタのクラス」まで巻き戻るため、
  派生クラスの override には決して届かない（派生部分はもう壊されている）。
  今のコードは葉のクラスばかりで実害ゼロだが、**派生を1つ足した瞬間に静かにリークする形**だった。
  修飾呼び出しは挙動を変えず、その事実をコードに書き留めるだけ
- **「検査を切る」と「指摘を直す」の線引きは、設計が動くかどうか。** 8/15 に CRTP を切ったのは
  `private` + `friend` が書籍サンプルの設計を書き換えるから。今回の `VirtualCall` は
  呼び先を明示するだけで設計も挙動も動かないので、同じ「書籍サンプル由来」でも結論が逆になった。
  **由来で決めず、変更が何を動かすかで決める**
- **同じ数字が2度出たら、それは測定が再現したということ。** 8/15 に scratchpad の
  コピーで測った10行が、実ツリーで内訳まで一致した。**設定を明示して測った測定は持ち越せる**
  （逆に、`-config-file` を忘れたときは 8件 → 85件に化けていた）
